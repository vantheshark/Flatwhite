using System.Collections.Generic;
using System.Runtime.Caching;

namespace Flatwhite.Tests
{
    /// <summary>
    /// Register this to the IOC container to override the global <see cref="ObjectCacheProvider"/> which used static ObjectCache that will break the unit tests
    /// </summary>
    public class UnitTestCacheProvider : ICacheProvider
    {
        private readonly IDictionary<string, object> _cache = new Dictionary<string, object>();
        public void Set(string key, object value, CacheItemPolicy policy)
        {
            _cache[key] = value;
        }

        public object Remove(string key)
        {
            if (Contains(key))
            {
                var obj = Get(key);
                _cache.Remove(key);
                return obj;
            }
            return null;
        }

        public object Get(string key)
        {
            return Contains(key) ? _cache[key] : null;
        }

        public bool Contains(string key)
        {
            return _cache.ContainsKey(key);
        }
    }
}
